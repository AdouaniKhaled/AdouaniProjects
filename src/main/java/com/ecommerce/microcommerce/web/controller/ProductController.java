...
public class ProductController {
    ...
    public ResponseEntity<Void> ajouterProduit(@RequestBody Product product) {
        ...
        if(product.getPrix()==0){
            throw new ProduitGratuitException("Le prix de vente doit être strictement positif");
        }
    ...
    public Map<Product, Integer> calculerMargeProduit() {
        ...
        while (it.hasNext()) {
            pdt = it.next();
            marge = pdt.getPrix() - pdt.getPrixAchat();
            result.put(pdt, marge);
        }
        return result;
    }
